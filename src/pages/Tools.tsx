import { Lock, Bug, Search, HardDrive, Usb, Download, Zap, Eye, Bitcoin, Terminal, Shield } from "lucide-react";
...
const tools = [
  ...
  },
  {
    ...
    category: "WEB ENGINE",
    desc: "Multi-modal AI fraud detection with three specialized modes: UPI QR Code Scam Inspector, Deepfake Voice Detector, and URL Phishing Analyzer with attack signatures.",
    version: "v1.0.1",
    platform: "Client-Side Browser · Zero Install",
  ...
  },
  {
    ...
    desc: "Zero-knowledge credential vault protected by PBKDF2-HMAC-SHA256 master key derivation + TOTP. Fernet-encrypted SQLite vault where the master key is never written to disk.",
    version: "v2.0.0",
  ...
  },
  {
    ...
    category: "STANDALONE DESKTOP",
    desc: "Deep directory auditor that crawls local disks, hashes all files, and flags malicious persistence mechanisms, hidden scripts, and suspicious extensions.",
    version: "v2.0.0",
    platform: "macOS · Windows Desktop",
    webTool: null,
  ...
  },
  {
    ...
    category: "STANDALONE DESKTOP",
    desc: "Real-time background monitor that detects USB devices the instant they connect or disconnect. Logs Vendor ID, Product ID, serial numbers, and device descriptions with audio and native OS alerts.",
    version: "v2.0.0",
    platform: "macOS · Windows Desktop",
    webTool: null,
    ...
    <div className="min-h-screen pt-24 pb-20 bg-background text-foreground">
      ...
      <section className="py-16 md:py-24 border-b border-border relative overflow-hidden">
        ...
        <div className="container relative z-10 max-w-4xl">
          ...
          <h1 className="text-3xl sm:text-5xl font-bold tracking-tight text-foreground leading-[1.1] mb-6 animate-fade-up" style={{ animationDelay: "100ms" }}>
            Specialized Tooling Engineered for Offensive &amp; Defensive Precision
          </h1>
          <p className="text-base sm:text-lg text-muted-foreground leading-relaxed animate-fade-up" style={{ animationDelay: "200ms" }}>
          ...
          {!user && (
            <div className="mt-6 inline-flex items-center gap-2.5 px-4 py-2.5 rounded-lg border border-yellow-500/30 bg-yellow-500/10 text-yellow-400 text-xs font-mono animate-fade-up" style={{ animationDelay: "300ms" }}>
              <Lock className="h-3.5 w-3.5 shrink-0" />
              Sign in to launch tools and download applications
              <Link to="/login" className="underline font-bold hover:text-yellow-300 transition-colors ml-1">Sign In →</Link>
            </div>
          )}
        </div>
      </section>
                ...
                <div className="p-6 sm:p-7 rounded-2xl border border-border bg-card/90 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 hover:border-primary/30 transition-colors shadow-sm">
                  ...
                  <div className="flex flex-wrap items-center gap-2.5 shrink-0 w-full md:w-auto">
                    <AuthGatedLink
                      ...
                      size="sm"
                      download={true}
                      className="bg-primary hover:bg-primary/90 text-primary-foreground font-semibold text-xs gap-1.5 shadow-sm inline-flex items-center justify-center rounded-md px-3 py-2"
                    >
                      <Download className="h-3.5 w-3.5 mr-1" /> macOS (.zip)
                    </AuthGatedLink>
                    ...
                    <AuthGatedLink
                      ...
                      variant="outline"
                      download={true}
                      className="border-border bg-card hover:bg-muted text-xs gap-1.5 shadow-sm inline-flex items-center justify-center rounded-md px-3 py-2"
                    >
                      <Download className="h-3.5 w-3.5 mr-1" /> Windows (.exe)
                    </AuthGatedLink>
                  </div>
                </div>
              </ScrollReveal>
